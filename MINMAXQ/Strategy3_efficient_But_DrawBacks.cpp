...
//    vector<int> chk;
	...
	if(arr[i]==arr[i-1])ct++;
	    ...
	    //chk.push_back(ct);
	    if(ct==1)ans++;
	    else pq.push({(ct*(ct+1))/2-ct,{0,ct}});
	    ct=1;
	}
    }
    if(ct==1)ans++;
	...
	pq.pop();
	tp.second.first++;
	int ss=tp.second.first,sf=tp.second.second;
	//tp.first=((sf-ss)/ss+(((sf-ss)%ss)?1:0));
	int sz=((sf-ss)/(ss+1));
	if((sf-ss)%(ss+1)){
	    int a = (sf-ss)%(ss+1);
	    int b = (ss+1-a);
	    tp.first=((b)*((sz*(sz+1))/2)+(a)*(((sz+1)*(sz+2))/2));
	}
	else {
	    tp.first=(ss+1)*((sz*(sz+1))/2);
	}
	tp.first+=ss;
	tp.first-=sf;
	pq.push(tp);
