...
void Mamba_Mentality(){
    ...
    vector<int> dp(nn,0);
    vector<int> dpp(nn,0);
    for(int i=0;i<nn;i++)dpp[i]=cost(chk[i],0);
    //trace(dpp);
    int sz=nn;
    for(int i=1;i<=k;i++){
	int mx_=0,idx=-1;
	for(int p=0;p<sz;p++){
	    int tp=dpp[p]-cost(chk[p],dp[p]+1);
	    if(mx_<tp){
		mx_=tp;
		idx=p;
	    }
	...
	if(mx_==0)break;
	    ...
	    dp[idx]++;
	    dpp[idx]=cost(chk[idx],dp[idx]);
	    if(dpp[idx]==chk[idx]){
		swap(dpp[idx],dpp[sz-1]);
		swap(dp[idx],dp[sz-1]);
		swap(chk[idx],chk[sz-1]);
		sz--;
	    }
	}
    }
    //trace(dpp);
    for(int i=0;i<nn;i++)ans+=dpp[i];
    cout<<ans<<"\n";
